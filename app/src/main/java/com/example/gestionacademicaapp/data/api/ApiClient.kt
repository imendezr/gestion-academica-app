...
object ApiClient {
    ...
    // Método genérico para solicitudes GET con reintentos
    suspend fun <T> get(context: Context, endpoint: String, responseClass: Class<T>): ApiResponse<T> =
        withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    if (responseCode == HttpURLConnection.HTTP_OK) {
                        ...
                        if (!isRetryableError(responseCode, null)) {
                            return@withContext ApiResponse.error("Error en la solicitud: $responseCode - $errorMessage", responseCode)
                        }
                    }
                } catch (e: Exception) {
                    if (!isRetryableError(null, e) || attempt == Constants.MAX_RETRIES - 1) {
                        return@withContext ApiResponse.error("Error de conexión: ${e.message}")
                    }
                } finally {
            ...
            return@withContext ApiResponse.error("Error: Se agotaron los intentos (${Constants.MAX_RETRIES})")
        }

    // Método genérico para solicitudes POST con reintentos
    suspend fun <T> post(context: Context, endpoint: String, data: Any, responseClass: Class<T>): ApiResponse<T> =
        withContext(Dispatchers.IO) {
            if (!context.isNetworkAvailable()) {
                return@withContext ApiResponse.error("No hay conexión a Internet")
            }
    ...
    // Método genérico para solicitudes PUT con reintentos
    suspend fun <T> put(context: Context, endpoint: String, data: Any, responseClass: Class<T>): ApiResponse<T> =
        withContext(Dispatchers.IO) {
            if (!context.isNetworkAvailable()) {
                return@withContext ApiResponse.error("No hay conexión a Internet")
            }
    ...
    // Método genérico para solicitudes DELETE con reintentos
        ...
        withContext(Dispatchers.IO) {
            ...
            while (attempt < Constants.MAX_RETRIES) {
                ...
                try {
                    ...
                    return@withContext ApiResponse.success(success)
                } catch (e: Exception) {
                    if (!isRetryableError(null, e) || attempt == Constants.MAX_RETRIES - 1) {
                        return@withContext ApiResponse.error("Error de conexión: ${e.message}")
                    }
